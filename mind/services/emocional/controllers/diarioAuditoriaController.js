...
class DiarioAuditoriaController {
  // Get all diary audit records with pagination and filtering
    ...
    try {
      ...
      if (req.query.fechaDesde || req.query.fechaHasta) {
        ...
        if (req.query.fechaHasta) filter.fecha.$lte = new Date(req.query.fechaHasta);
            ...
            const auditorias = await DiarioAuditoria.find(filter)
      ...
      res.status(500).json({
        success: false,
        message: 'Error al obtener registros de auditoría del diario',
        error: error.message
      });
    }
  }
  ...
  static async getById(req, res) {
    try {
      const auditoria = await DiarioAuditoria.findById(req.params.id);
  ...
  static async getByEntidad(req, res) {
    try {
      ...
      const auditorias = await DiarioAuditoria.find({
        entidad: entidad,
        idEntidad: idEntidad
      }).sort({ fecha: -1 });
  ...
  static async getStats(req, res) {
    try {
      const stats = await DiarioAuditoria.aggregate([
        {
          $group: {
            _id: {
              entidad: '$entidad',
  ...
  static async cleanup(req, res) {
    try {
      ...
      const result = await DiarioAuditoria.deleteMany({
        fecha: { $lt: cutoffDate }
      });
...
module.exports = DiarioAuditoriaController;
